...
这段文字会显示在 MoviePilot 插件页的「更新历史」里，也会当成 GitHub Release 正文。读它的人
是在决定要不要升级的用户，不是在做事后复盘的开发者。

- **一条 60 字上下，最多三四句**。仓库里的老条目大多 20-60 字，那就是对的尺度。
- **先说对用户有什么变化**，不写根因链条、不写代码符号与函数名、不画图。
- 并列的改动用 `；` 隔开，整条以 `。` 收尾；引号用 `「」`。
- 数字两边留空格（`最近 6 条`、`1.55MB → 44KB`），和界面文案同一套排版规矩。
- 修复类直接写「修好 / 修复 + 用户看得见的症状」，比如「修好『有些天不签到』」比
  「修复 APScheduler misfire_grace_time 导致的触发丢弃」有用。

排查过程、根因、取舍这些留给 commit message —— 那里是给读代码的人看的，篇幅不限。
