...
class OpenListManager(_PluginBase):
    ...
    def _save_config(self, config_payload: dict) -> Dict[str, Any]:
        ...
        try:
            # Update instance variables directly from payload
    ...
    def _get_status(self) -> Dict[str, Any]:
        """API Endpoint: Returns current plugin status."""
        return {
            "status": self._task_status.get("status", "idle"),
            "progress": self._task_status.get("progress", 0),
            "last_run": self._task_status.get("last_run"),
            "total_files": self._task_status.get("total_files", 0),
            "copied_files": self._task_status.get("copied_files", 0),
            "skipped_files": self._task_status.get("skipped_files", 0),
            "target_files_count": self._target_files_count
        }
    
    def _run_task(self) -> Dict[str, Any]:
        """API Endpoint: Triggers a manual copy task."""
        logger.info("收到手动执行复制任务请求")
        if self._task_status.get("status") == "running":
            return {"message": "任务正在执行中", "error": True}
    ...
    def get_form(self) -> Tuple[Optional[List[dict]], Dict[str, Any]]:
        """Vue mode doesn't use Vuetify form definitions."""
