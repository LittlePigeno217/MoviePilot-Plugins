    ...
    def _ensure_directory_exists(self, path: str) -> bool:
        ...
        try:
            ...
            if normalized_path.startswith('/'):
                path_parts.append('')  # 保留开头的空部分，代表根目录
            # 分割路径
            path_parts.extend(normalized_path.split('/'))
            # 从根目录开始，逐层创建目录
            current_path = ""
            for part in path_parts:
                if not part and current_path:  # 跳过中间的空部分（处理//的情况）
                    continue
                ...
                # 检查当前路径是否存在
                check_params = {"path": current_path}
                check_response = requests.post(url, headers=headers, json=check_params, timeout=10)
                if check_response.status_code == 200:
                    check_result = check_response.json()
                    if check_result.get("code") == 200:
                        continue  # 目录已存在，继续下一层
                
                # 当前目录不存在，创建它
                mkdir_url = f"{self._openlist_url}/api/fs/mkdir"
                mkdir_data = {"path": current_path}
                
                mkdir_response = requests.post(mkdir_url, headers=headers, json=mkdir_data, timeout=10)
                if mkdir_response.status_code == 200:
                    mkdir_result = mkdir_response.json()
                    if mkdir_result.get("code") != 200:
                        logger.warning(f"创建目录 {current_path} 失败: {mkdir_result.get('message')}")
                        return False
                else:
                    logger.warning(f"创建目录请求失败，状态码: {mkdir_response.status_code}")
                    return False
