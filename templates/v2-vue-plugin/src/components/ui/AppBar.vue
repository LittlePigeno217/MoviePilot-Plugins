 ...
 *
 * 三块：身份（色块 + 名称 + 视图名）、状态、工具（刷新 / 切换 / 关闭）。
 * 右对齐靠身份块 `flex: 1 1 auto` 吃掉空白，**不要**靠状态那一格的
 * `margin-inline-start: auto` —— 状态格在 ≤560px 时会被隐藏，auto 外边距
 * 跟着消失，三个按钮就滑到中间去了。见 docs/Plugin_UI_Spec.md 第 4.1 节。
 */
defineProps({
...
<template>
  <header class="bar">
    <span class="bar__mark" aria-hidden="true">插</span>

    <span class="bar__names">
      <span class="bar__name">插件名</span>
      <span class="bar__view tpl-label">{{ view }}</span>
    </span>
